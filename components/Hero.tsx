...
const [whichCan, setWhichCan] = useState('peachCan1');
  ...
  let topicName;
  let textColor;
  let collectionCan = ['peachCan1', 'peachCan2', 'peachCan3', 'peachCan4'];
  if (theme === 'orange') {
    leftTheme = 'bg-[#ff8c00]';
        ...
        <div className='flex flex-col items-center h-[80%] p-[5rem]'>
          ...
          <div className='flex items-center mt-[2.5rem] space-x-4 absolute top-[25rem]'>
            {listCan.map((can, index) => {
              return (
                <div className='flex-col space-y-3 items-center' key={index}>
          ...
          <div className='border border-zinc-300 w-[28%] h-[18%] absolute top-[38rem] rounded-lg pt-[0.8rem] pl-[1.2rem]'>
            <div className='flex items-center text-white text-[20px] w-[95%]'>
              <div
                className={`flex w-[50%] justify-center cursor-pointer ${activeButton === 'BUY NOW' ? 'bg-[#ffa131] text-white rounded-lg' : ''}`}
                onClick={() => setActiveButton('BUY NOW')}
              >
                BUY NOW
              </div>
              <div
                className={`flex w-[50%] justify-center cursor-pointer ${activeButton === 'FIND IN STORES' ? 'bg-[#ffa131] text-white rounded-lg' : ''}`}
